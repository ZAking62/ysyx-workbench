...
// #define ECALL(dnpc) { dnpc = (isa_raise_intr(isa_query_intr(), s->pc)); }   有问题 ，异常号和系统调用号混一起了
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
	INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh		 , I, R(rd) = SEXT(Mr(src1 + imm, 2), 16));//1
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));
	...
	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu	 , R, R(rd) = ((uint64_t)src1 * (uint64_t)src2) >> 32);//1
